...
% echo in chunk (para que no se vea el codigo)
% results in latex (results=tex) para que sepa que tienen que generarse en latex
% par(mfrow=c(filas,columnas))
% stargazer and report table packages

\usepackage[utf8]{inputenc} % ayuda con los acentos
...
library(reporttools) 
library(xtable)
# tableNominal viene de reporttools que necesita xtable
# 
